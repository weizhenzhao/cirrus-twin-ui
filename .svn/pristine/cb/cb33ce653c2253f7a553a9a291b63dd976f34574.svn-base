define(['angular', './px-module'], function(angular, pxModule) {
    ...
    return pxModule.controller('ChartCtrl', ['$scope','$http','$stateParams', function($scope,$http,$stateParams) {
        ...
        $scope.sendMsg= function(){
            var preTxt = preTxtPre+ $("#phraseDiv").val()+preTxtSuf;
            if($("#phraseDiv").val()!=''){
            ...
            if(isMuted == 'false'){
                $http({
                    url: 'api/CirrusTwin/getAnswerByText2Voice/'+$("#phraseDiv").val()+'/'+$('#gender').val()+'/',
                    method: 'GET'
                ...
                $http({
                    url: 'api/CirrusTwin/getAnswerByText/'+$("#phraseDiv").val()+'/',
                    method: 'GET'
                }).success(function (data, header, config, status) {
